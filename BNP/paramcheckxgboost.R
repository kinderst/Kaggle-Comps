...
max_depth: 6, 9, 12
tree depth size
min_child_weight: 0, 1 (default), 10
...
maxDeltaResults <- c()
subsampleResults <- c()
colSubsampleResults <- c()


#etas <- c(0.005, 0.01, 0.1)
#gammas <- c(0, 0.5, 3)
#maxDepths <- c(7, 9, 11)
#minChilds <- c(1)
#maxDeltas <- c(1)
#subsamples <- c(0.9)
colSubsamples <- c(0.5, 0.7, 0.9, 1)
start.time <- Sys.time()
for (colSubsample in colSubsamples) {
  params0 <- list(
  ...
  rm(model_cv)
  colSubsampleResults <- c(colSubsampleResults, paste("colSubsample: ", colSubsample, "logloss: ", best, "bestiter: ", bestIter, sep = ", "))
}
end.time <- Sys.time()
